...
// the generator would have written non-go code to the _tndr.txt file, which
// is then read by this function and written to the output.
//
// Deprecated: since tndr v0.3.x generated code uses WriteString.
func WriteWatchModeString(w io.Writer, lineNum int) error {
	_, path, _, _ := runtime.Caller(1)
	if !strings.HasSuffix(path, "_tndr.go") {
		return errors.New("tndr: WriteWatchModeString can only be called from _tndr.go")
	}
	txtFilePath := strings.Replace(path, "_tndr.go", "_tndr.txt", 1)

	literals, err := getWatchedStrings(txtFilePath)
	if err != nil {
		return fmt.Errorf("tndr: failed to cache strings: %w", err)
	}

	if lineNum > len(literals) {
		return fmt.Errorf("tndr: failed to find line %d in %s", lineNum, txtFilePath)
	}
