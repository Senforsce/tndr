...
func TestHandler(t *testing.T) {
	hello := tndr.ComponentFunc(func(ctx context.Context, w io.Writer) error {
		if _, err := io.WriteString(w, "Hello"); err != nil {
			t.Fatalf("failed to write string: %v", err)
		}
		return nil
	})
	errorComponent := tndr.ComponentFunc(func(ctx context.Context, w io.Writer) error {
		if _, err := io.WriteString(w, "Hello"); err != nil {
			t.Fatalf("failed to write string: %v", err)
		}
		return errors.New("handler error")
	})
	fragmentPage := tndr.ComponentFunc(func(ctx context.Context, w io.Writer) error {
		if _, err := io.WriteString(w, "page_contents\n"); err != nil {
			return err
		}
		fragmentContents := tndr.ComponentFunc(func(ctx context.Context, w io.Writer) error {
			_, err := io.WriteString(w, "fragment_contents")
			return err
		})
		return tndr.Fragment("fragment").Render(tndr.WithChildren(ctx, fragmentContents), w)
	})

	type nameFragmentKey struct{}
	var fragmentContentsName = nameFragmentKey{}
	fragmentPageWithCustomKey := tndr.ComponentFunc(func(ctx context.Context, w io.Writer) error {
		if _, err := io.WriteString(w, "page_contents\n"); err != nil {
			return err
		}
		fragmentContents := tndr.ComponentFunc(func(ctx context.Context, w io.Writer) error {
		...
		return tndr.Fragment(fragmentContentsName).Render(tndr.WithChildren(ctx, fragmentContents), w)
	})

	tests := []struct {
		name             string
		input            *tndr.ComponentHandler
		expectedStatus   int
		...
		{
			name:             "handlers return OK by default",
			input:            tndr.Handler(tndr.Raw(`♠ ‘ &spades; &#8216;`)),
			expectedStatus:   http.StatusOK,
			expectedMIMEType: "text/html; charset=utf-8",
			expectedBody:     "♠ ‘ &spades; &#8216;",
		},
		{
			...
			input:            tndr.Handler(hello, tndr.WithStatus(http.StatusNotFound)),
		...
		},
		{
			...
			input:            tndr.Handler(hello, tndr.WithStatus(http.StatusOK), tndr.WithContentType("text/csv")),
			expectedStatus:   http.StatusOK,
		...
		},
		{
			...
			input:            tndr.Handler(errorComponent),
			expectedStatus:   http.StatusInternalServerError,
			expectedMIMEType: "text/plain; charset=utf-8",
			expectedBody:     "tndr: failed to render template\n",
		},
		{
			name: "error handling can be customised",
			input: tndr.Handler(errorComponent, tndr.WithErrorHandler(func(r *http.Request, err error) http.Handler {
		...
		},
		{
			...
			input:            tndr.Handler(fragmentPage, tndr.WithFragments("fragment")),
			expectedStatus:   http.StatusOK,
			expectedMIMEType: "text/html; charset=utf-8",
		...
		},
		{
			...
			input:            tndr.Handler(fragmentPageWithCustomKey, tndr.WithFragments(fragmentContentsName)),
			expectedStatus:   http.StatusOK,
			expectedMIMEType: "text/html; charset=utf-8",
			expectedBody:     "fragment_contents",
		},
		{
			...
			input:            tndr.Handler(fragmentPage),
			expectedStatus:   http.StatusOK,
			expectedMIMEType: "text/html; charset=utf-8",
		...
		},
		{
			...
			input:            tndr.Handler(fragmentPage, tndr.WithFragments("fragment"), tndr.WithStreaming()),
			expectedStatus:   http.StatusOK,
			expectedMIMEType: "text/html; charset=utf-8",
			expectedBody:     "fragment_contents",
		},
	...
	t.Run("streaming mode allows responses to be flushed", func(t *testing.T) {
		...
		component := tndr.ComponentFunc(func(ctx context.Context, w io.Writer) error {
			// Write part 1.
		...
		tndr.Handler(component, tndr.WithStatus(http.StatusCreated), tndr.WithStreaming()).ServeHTTP(w, r)
	...
	t.Run("streaming mode handles errors", func(t *testing.T) {
		...
		h := tndr.Handler(component,
			tndr.WithStatus(http.StatusCreated),
			tndr.WithStreaming(),
			tndr.WithErrorHandler(errorHandler),
		)
		h.ServeHTTP(w, r)
