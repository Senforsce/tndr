...
const y = ` + "`456`"
	...
	// https://github.com/senforsce/tndr/issues/505
...
t1 template(
	...
	t.Run("as you type a t1 file, it parses as much as it can, even if there's an error, so that the LSP functions", func(t *testing.T) {
		input := `package main
...
t1 Hello(name string) {
  if nam`
		...
		// Expect the range of the HTML template to be from `t1 Hello` to the end of the input.
		expectedRange := Range{
