...
// Inside a call to a tndrate, passing some data.
...
func TestExpressions(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "tndr: map parameter",
			input:           `{ t1 TemplName(data map[string]any) }`,
			prefix:          "{ t1 ",
			startBraceCount: 1,
