...
func Test(t *testing.T) {
	...
	t.Run("attribute expressions can return errors", func(t *testing.T) {
		...
		t.Run("the errors are tndr errors", func(t *testing.T) {
			var templateErr tndr.Error
			if !errors.As(err, &templateErr) {
				t.Fatalf("expected error to be tndr.Error, but got %T", err)
			}
			if templateErr.FileName != `generator/test-attribute-errors/template.t1` {
				t.Errorf("expected error in `generator/test-attribute-errors/template.t1`, but got %v", templateErr.FileName)
			}
