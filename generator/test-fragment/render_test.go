...
func Test(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			handler:  tndr.Handler(Page(), tndr.WithFragments("content-a")),
			expected: `<div>Fragment Content A</div>`,
		},
		{
			name:     "multiple fragments can be rendered",
			handler:  tndr.Handler(Page(), tndr.WithFragments("content-a", "content-b")),
		...
		},
		{
			...
			handler:  tndr.Handler(Page(), tndr.WithFragments("outer")),
		...
		},
		{
			...
			handler:  tndr.Handler(Page(), tndr.WithFragments("inner")),
			expected: `<div>Inner Fragment Content</div>`,
		},
		{
			name:     "fragments that don't exist return an empty string",
			handler:  tndr.Handler(Page(), tndr.WithFragments("non-existent")),
			expected: "",
		},
	}
