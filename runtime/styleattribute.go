...
// - tndr.KeyValue[string, string] - A map of key/values where the key is the CSS property name and the value is the CSS property value.
// - tndr.KeyValue[string, tndr.SafeCSSProperty] - A map of key/values where the key is the CSS property name and the value is the CSS property value.
// - tndr.KeyValue[string, bool] - The bool determines whether the value should be included.
// - tndr.KeyValue[tndr.SafeCSS, bool] - The bool determines whether the value should be included.
// - func() (anyOfTheAboveTypes)
// - func() (anyOfTheAboveTypes, error)
// - []anyOfTheAboveTypes
//
// In the above, tndr.SafeCSS and tndr.SafeCSSProperty are types that are used to indicate that the value is safe to render as CSS without sanitization.
...
func sanitizeStyleAttributeValue(sb *strings.Builder, v any) error {
	...
	case map[string]tndr.SafeCSSProperty:
		return processSafeCSSPropertyMap(sb, v)

	case tndr.KeyValue[string, string]:
		return processStringKV(sb, v)

	case tndr.KeyValue[string, bool]:
		if v.Value {
			return processString(sb, v.Key)
		}
		return nil

	case tndr.KeyValue[tndr.SafeCSS, bool]:
		if v.Value {
...
func processSafeCSS(sb *strings.Builder, v tndr.SafeCSS) error {
	if v == "" {
		return nil
	}
...
// processSafeCSSPropertyMap processes a map[string]tndr.SafeCSSProperty.
func processSafeCSSPropertyMap(sb *strings.Builder, m map[string]tndr.SafeCSSProperty) error {
	for _, name := range slices.Sorted(maps.Keys(m)) {
...
// processStringKV processes a tndr.KeyValue[string, string].
func processStringKV(sb *strings.Builder, kv tndr.KeyValue[string, string]) error {
