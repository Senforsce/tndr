...
func getTemplInfo() (d ToolInfo) {
	...
	// Run tndr to find the version.
	cmd := exec.Command(d.Location, "version")
	v, err := cmd.Output()
	if err != nil {
		d.Message = fmt.Sprintf("failed to get tndr version: %v", err)
		return
	}
	d.Version = strings.TrimSpace(string(v))
	if d.Version != tndr.Version() {
		d.Message = fmt.Sprintf("version mismatch - you're running %q at the command line, but the version in the path is %q", tndr.Version(), d.Version)
		return
	}
	d.OK = true
	return
...
func findTempl() (location string, err error) {
	...
	if !errors.Is(err, exec.ErrNotFound) {
		return "", fmt.Errorf("unexpected error looking for tndr: %w", err)
	}

	return "", fmt.Errorf("tndr is not in the path (%q). You can install tndr with `go install github.com/senforsce/tndr/cmd/tndr@latest`", os.Getenv("PATH"))
}
...
func Run(ctx context.Context, log *slog.Logger, stdout io.Writer, args Arguments) (err error) {
	...
	logInfo(ctx, log, "tndr", info.Tndr)
	return nil
}
