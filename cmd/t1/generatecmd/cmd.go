...
func (cmd Generate) Run(ctx context.Context) (err error) {
	...
	if cmd.Args.IncludeVersion {
		opts = append(opts, generator.WithVersion(tndr.Version()))
	}
	if cmd.Args.IncludeTimestamp {
		opts = append(opts, generator.WithTimestamp(time.Now()))
	}

	// Check the version of the tndr module.
	if err := modcheck.Check(cmd.Args.Path); err != nil {
		cmd.Log.Warn("tndr version check: " + err.Error())
	}
...
loop:
	for {
		...
		case ge := <-postGeneration:
			...
			grouped.TndrFileTextUpdated = grouped.TndrFileTextUpdated || ge.TndrFileTextUpdated
			grouped.TndrFileGoUpdated = grouped.TndrFileGoUpdated || ge.TndrFileGoUpdated
			if grouped.WatchedFileUpdated || grouped.TndrFileTextUpdated || grouped.TndrFileGoUpdated {
				updates++
			}
		...
		case <-timeout.C:
			...
			if grouped == nil || (!grouped.WatchedFileUpdated && !grouped.TndrFileTextUpdated && !grouped.TndrFileGoUpdated) {
				timeout = time.NewTimer(time.Hour * 24 * 365)
				continue loop
			}
...
loop:
	for {
		...
		// The Go application needs to be restarted if any watched non-tndr watched files (i.e. non-tndr Go files)
		// were updated, or if any Go code within a tndr file was updated.
		needsRestart := grouped.WatchedFileUpdated || grouped.TndrFileGoUpdated
		// If the text in a tndr file, or any other changes have happened, reload the browser.
		needsBrowserReload := grouped.TndrFileTextUpdated || grouped.TndrFileGoUpdated || grouped.WatchedFileUpdated
		...
		if cmd.Args.Command != "" && needsRestart {
			...
			if cmd.Args.Watch {
				if err := os.Setenv("TNDR_DEV_MODE", "true"); err != nil {
					cmd.Log.Error("Error setting TNDR_DEV_MODE environment variable", slog.Any("error", err))
				}
			}
...
func (cmd Generate) handleEvents(ctx context.Context, events chan fsnotify.Event, errs chan error, fseh *FSEventHandler, postGeneration chan *GenerationEvent) {
	...
	for event := range events {
		...
		go func(event fsnotify.Event) {
			...
			if !r.WatchedFileUpdated && !r.TndrFileTextUpdated && !r.TndrFileGoUpdated {
			...
			e := &GenerationEvent{
				...
				TndrFileTextUpdated: r.TndrFileTextUpdated,
				TndrFileGoUpdated:   r.TndrFileGoUpdated,
			}
...
func (cmd *Generate) deleteWatchModeTextFiles() error {
	return fs.WalkDir(os.DirFS(cmd.Args.Path), ".", func(path string, info os.DirEntry, err error) error {
		...
		if !strings.HasSuffix(absPath, "_t1.go") && !strings.HasSuffix(absPath, ".t1") {
			return nil
		}
		watchModeFileName := runtime.GetDevModeTextFileName(absPath)
