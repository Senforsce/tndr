...
import (
	...
	"github.com/senforsce/tndr/cmd/t1/lspcmd/lspdiff"
...
func TestReferences(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// this is the definition of the tndr function in the templates.t1 file.
			line:      5,
			character: 9,
			filename:  "/templates.t1",
			assert: func(t *testing.T, actual []protocol.Location) (msg string, ok bool) {
				expectedReference := []protocol.Location{
					{
						// This is the usage of the tndr function in the main.go file.
		...
		{
			// this is the definition of the struct in the templates.t1 file.
			line:      21,
			character: 9,
			filename:  "/templates.t1",
			assert: func(t *testing.T, actual []protocol.Location) (msg string, ok bool) {
				expectedReference := []protocol.Location{
					{
						// This is the usage of the struct in the templates.t1 file.
						URI: uri.URI("file://" + appDir + "/templates.t1"),
						Range: protocol.Range{
							Start: protocol.Position{
								Line:      uint32(24),
		...
		},
		{
			...
			line:      3,
			character: 9,
			filename:  "/remotechild.t1",
			assert: func(t *testing.T, actual []protocol.Location) (msg string, ok bool) {
				expectedReference := []protocol.Location{
					{
						URI: uri.URI("file://" + appDir + "/remoteparent.t1"),
						Range: protocol.Range{
							Start: protocol.Position{
								Line:      uint32(3),
								Character: uint32(2),
	...
	tests := []struct {
	...
	}{
		{
			...
			assert: func(t *testing.T, actual []protocol.CodeAction) (msg string, ok bool) {
				...
				// To support code actions, update cmd/tndr/lspcmd/proxy/server.go and add the
...
func Setup(ctx context.Context, log *slog.Logger) (clientCtx context.Context, appDir string, client protocol.Client, server protocol.Server, teardown func(t *testing.T), err error) {
	...
	if err != nil {
		return ctx, appDir, client, server, teardown, fmt.Errorf("could not find local tndr go.mod file: %v", err)
	}
	...
	if ir.ServerInfo.Name != "tndr-lsp" {
		return ctx, appDir, client, server, teardown, fmt.Errorf("expected server name to be tndr-lsp, got %q", ir.ServerInfo.Name)
	}
