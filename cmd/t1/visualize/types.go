...
func HTML(tndrFileName string, tndrContents, goContents string, sourceMap *parser.SourceMap) tndr.Component {
	tl := tndrLines{contents: string(tndrContents), sourceMap: sourceMap}
	gl := goLines{contents: string(goContents), sourceMap: sourceMap}
	return combine(tndrFileName, tl, gl)
}

type tndrLines struct {
	contents  string
	sourceMap *parser.SourceMap
}

func (tl tndrLines) Render(ctx context.Context, w io.Writer) (err error) {
	tndrLines := strings.Split(tl.contents, "\n")
	for lineIndex, line := range tndrLines {
...
func (gl goLines) Render(ctx context.Context, w io.Writer) (err error) {
	tndrLines := strings.Split(gl.contents, "\n")
	for lineIndex, line := range tndrLines {
		if _, err = w.Write([]byte("<span>" + strconv.Itoa(lineIndex) + "&nbsp;</span>\n")); err != nil {
			return
		}
