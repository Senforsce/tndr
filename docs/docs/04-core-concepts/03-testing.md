...
The example at https://github.com/senforsce/tndr/blob/main/examples/blog/posts_test.go shows how to test that a list of posts is rendered correctly.
...
Pages are also tndr components, so the tests are structured in the same way.
...
- goquery can be used effectively with tndr for writing component level tests.
...
tndr uses this strategy to check for regressions in behaviour between releases, as per https://github.com/senforsce/tndr/blob/main/generator/test-html-comment/render_test.go

To make it easier to compare the output against the expected HTML, tndr uses a HTML formatting library before executing the diff.
