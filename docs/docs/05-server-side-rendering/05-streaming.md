...
The default behaviour of the `tndr.Handler` is to render the template to a buffer and then write the buffer to the response.
...
Typical usage of tndr involves collecting data that is used to populate the template, before rendering the template and sending a response.
...
Streaming can be enabled by setting the `Streaming` field of the `tndr.Handler` to `true` using the `WithStreaming` option.

```go
tndr.Handler(component, tndr.WithStreaming()).ServeHTTP(w, r)
```

When streaming is enabled, sections of the template can be forcefully pushed to the client using the `tndr.Flush()` component.
...
By using `tndr.Flush()` to create a flushable area, the data is pushed to the client as soon as it is available, rather than waiting for the entire template to render before sending a response.

```tndr
t1 Page(data chan string) {
	<!DOCTYPE html>
	<html>
		<head>
			<title>Page</title>
...
See https://github.com/senforsce/tndr/tree/main/examples/streaming for a full example.
...
First, we need to define a new tndr component called `Slot`.
...
We need to use `@tndr.Flush()` to create a flushable area, so that the data is pushed to the client as soon as it is available, since popluating the slots will take longer to complete.
...
```tndr
t1 Page(data chan SlotContents) {
	<!DOCTYPE html>
	<html>
		<head>
			<title>Page</title>
...
See https://github.com/senforsce/tndr/tree/main/examples/suspense for a full working example.
