...
If you have `prettierd`, `prettier` or `npx` on your `PATH`, `tndr` will use it to format the `<script>` tag contents.

:::tip
To ensure that a `<script>` tag within a tndr component is only rendered once per HTTP response (or context), use a [tndr.OnceHandle](18-render-once.md).

Using a `tndr.OnceHandle` allows a component to define global client-side scripts that it needs to run without including the scripts multiple times in the response.
:::
...
HTML element `on*` attributes pass an event object to the function. To pass the event object to a function, use `tndr.JSExpression`.
...
Use `tndr.JSFuncCall` to call a client-side function with server-side data.
...
:::tip
If you want to write out an arbitrary string containing JavaScript, and are sure it is safe, you can use `tndr.JSUnsafeFuncCall` to bypass script sanitization.

Whatever string you pass to `tndr.JSUnsafeFuncCall` will be output directly to the HTML, so be sure to validate the input.
:::
...
t1 will automatically escape the Go data to prevent XSS attacks.
...
And use the imported JavaScript directly in tndr via `<script>` tags.
...
https://github.com/senforsce/tndr/tree/main/examples/typescript contains a TypeScript example that uses `esbuild` to transpile TypeScript into plain JavaScript, along with any required `npm` modules.
...
Use the `tndr.JSFuncCall`, `tndr.JSONString` and other features of tndr alongside standard `<script>` tags to import standalone JavaScript files, optionally created by a bundler like `esbuild`.
:::
...
`script` elements are tndr Components, so you can also directly render the Javascript function, passing in Go data, using the `@` expression:
...
The `JSExpression` type is used to pass arbitrary JavaScript expressions to a tndr script template.
