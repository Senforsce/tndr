...
The `tndr.Fragment` component can be used to render a subsection of a template, discarding all other output.

Fragments work well as an optimisation for HTMX, as discussed in https://htmx.org/essays/template-fragments/

## Define fragments

Define a fragment with `@tndr.Fragment("name")`, where `"name"` is the identifier for the fragment.
...
To render only the "name" fragment from the `Page` template, use the `tndr.WithFragments("name")` option when creating the HTTP handler:

```go title="main.go"
handler := tndr.Handler(Page(), tndr.WithFragments("name"))
http.Handle("/", handler)
```
...
If the `tndr.WithFragments("name")` option is omitted, the whole page is rendered as normal.

```go title="main.go"
handler := tndr.Handler(Page())
http.Handle("/", handler)
```
...
To use outside of an HTTP handler, e.g. when generating static content, you can render fragments with the `tndr.RenderFragments` function.

```go
w := new(bytes.Buffer)
if err := tndr.RenderFragments(context.Background(), w, fragmentPage, "name"); err != nil {
...
func handleRequest(w http.ResponseWriter, r *http.Request) {
  ...
  var opts []func(*tndr.ComponentHandler)
  if templateName := r.URL.Query().Get("template"); templateName != "" {
    opts = append(opts, tndr.WithFragments(templateName))
  }

  // Render the template or fragment and serve it.
  tndr.Handler(Page(state), opts...).ServeHTTP(w, r)
}
