...
tndr elements can have HTML attributes that use the double quote character `"`.
...
:::note
tndr is aware that `<hr/>` is a void element, and renders `<hr>` instead.
:::


To set boolean attributes using variables or tndrate parameters, a question mark after the attribute name is used to denote that the attribute is boolean.
...
Use an `if` statement within a tndr element to optionally add attributes to elements.
...
It's possible to spread any variable of type `tndr.Attributes`. `tndr.Attributes` is a `map[string]any` type definition.
...
* If the value is a `tndr.KeyValue[string, bool]`, the attribute is added if the boolean is true, e.g. `<div name="value">`.
* If the value is a `tndr.KeyValue[bool, bool]`, the attribute is added if both boolean values are true, as `<div name>`.

```t1
t1 component(shouldBeUsed bool, attrs tndr.Attributes) {
  <p { attrs... }>Text</p>
  <hr
    if shouldBeUsed {
      { attrs... }
...
tndr usage() {
  @component(false, tndr.Attributes{"data-testid": "paragraph"}) 
}
```
...
When you pass a `string` to these attributes, tndr will automatically sanitize the input URL, ensuring that the protocol is safe (e.g., `http`, `https`, or `mailto`) and does not contain potentially harmful protocols like `javascript:`.

:::caution
To bypass URL sanitization, you can use `tndr.SafeURL(myURL)` to mark that your string is safe to use.

This may introduce security vulnerabilities to your program.
:::

If you use a constant value, e.g. `<a href="javascript:alert('hello')">`, tndr will not modify it, and it will be rendered as is.

:::tip
Non-standard HTML attributes can contain URLs, for example HTMX's `hx-*` attributes).

To sanitize URLs in that context, use the `tndr.URL(urlString)` function.

```t1
t1 component(contact model.Contact) {
  <div hx-get={ tndr.URL(fmt.Sprintf("/contacts/%s/email", contact.ID)) }>
    { contact.Name }
  </div>
}
```
...
:::note
In tndr, all attributes are HTML-escaped. This means that:
...
Sanitization is the process of examining the URL scheme (protocol) and structure to ensure that it's safe to use, e.g. that it doesn't contain `javascript:` or other potentially harmful schemes. If a URL is not safe, tndr will replace the URL with `about:invalid#TndrFailedSanitizationURL`.
:::

## JavaScript attributes

`onClick` and other `on*` handlers have special behaviour, they expect a reference to a `script` tndrate.
...
```html title="Output"
<script>
 function __tndr_withParameters_1056(a, b, c){console.log(a, b, c);}function __tndr_withoutParameters_6bbf(){alert("hello");}
</script>
<button onclick="__tndr_withParameters_1056("test","Say hello",123)" onmouseover="__tndr_withoutParameters_6bbf()" type="button">
 Say hello
</button>
```
