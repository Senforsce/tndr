...
To use React components in your tndr app, create your React components using TSX (TypeScript) or JSX as usual.
...
Next, use tndr to create a page containing HTML elements with specific IDs.
...
:::tip
Remember to run `tndr generate` when you've finished writing your tndr file.
:::
...
// Render the React component into the tndr page at the react-header.
...
### Serve the tndr component and client side bundle

To serve the server-side rendered tndr template, and the client-side JavaScript bundle created in the previous step, setup a Go web server.
...
### Update the tndr component to use the new function

Now that we have a `renderHello` function that will render the React component to the given element, we can update the tndr components to use it.
