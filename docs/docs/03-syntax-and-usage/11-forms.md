...
t1 can be used to create forms that submit data to the server. Depending on the design of your app, you can collect data from the form using JavaScript and submit it to an API from the frontend, or use a HTTP form submission to send the data to the server.

## Hypermedia approach

t1 isn't a framework, you're free to choose how you want to build your applications, but a common approach is to create a handler for each route, and then use templates to render the form and display validation errors.
...
If a field value is invalid, the `has-error` class is added to the form group using the `tndr.KV` function.
...
For full example code, see `./examples/crud` in `github.com/senforsce/tndr`.
...
t1 ContactList(contacts []db.Contact) {
  <table class="table">
  <tr>
    <th>
      Name
...
t1 ValidationMessages(msgs []string) {
  if len(msgs) > 0 {
