...
import "fmt"
import "github.com/senforsce/tndr/cmd/t1/testproject/css-classes"
...
`,
			assertions: func(t *testing.T, updated string) {
				if count := strings.Count(updated, "github.com/senforsce/tndr/cmd/t1/testproject/css-classes"); count != 0 {
...
import "fmt"
import  cssclasses "github.com/senforsce/tndr/cmd/t1/testproject/css-classes"
...
`,
			assertions: func(t *testing.T, updated string) {
				if count := strings.Count(updated, "cssclasses \"github.com/senforsce/tndr/cmd/t1/testproject/css-classes\""); count != 1 {
					t.Errorf("expected named import to be retained, got %d instances of it", count)
				}
				if count := strings.Count(updated, "github.com/senforsce/tndr/cmd/t1/testproject/css-classes"); count != 1 {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			// Load the templates.t1 file.
			filePath := path.Join(dir, "templates.t1")
