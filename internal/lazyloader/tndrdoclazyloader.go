...
type TemplDocLazyLoader interface {
	...
	// Sync syncs the dependencies of a t1 document using the changes made to the document.
	Sync(ctx context.Context, params *lsp.DidChangeTextDocumentParams) error

	// Unload unloads a t1 document and its dependencies.
	Unload(ctx context.Context, params *lsp.DidCloseTextDocumentParams) error

	// HasLoaded reports whether a t1 document and its dependencies have been loaded.
